classdef ColorsRGBApp < handle
    ...
    properties (Constant)
        AppPathBase = {'/ColorsRGB'};
        AppClass = 'ColorsRGBApp';
        Increment = 1;
        Decrement = 0;
        Version = '13a';
    end
    ...
    methods
        ...
        function startApp(obj)
            ...
            mlock;
            ColorsRGBApp.refcount(obj.Increment);
            ...
            existVal = exist(fullfile(pwd,'ColorsRGB')); %#ok<EXIST>
            ...
            if (doesShadowExist && ~pathOne.equals(pathTwo))
                % We are trying to execute the wrong ColorsRGB
                errordlg(message('MATLAB:apps:runapp:WrongEntryPoint', 'ColorsRGB').getString, ...
            ...
            if nargout(@ColorsRGB) == 0
                eval('ColorsRGB');
            else
                obj.AppHandle = eval('ColorsRGB');
            end
